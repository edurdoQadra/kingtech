...
class TransactionController extends Controller
{
    ...
    {
        $month = $request->input('month'); // Número del mes: 1 para enero, 2 para febrero, etc.
        $year = $request->input('year');   // Año para filtrar (opcional)

        if (!is_numeric($month) || $month < 1 || $month > 12) {
            return response()->json(['error' => 'Invalid month number.'], 400);
        }

        $year = $year ?: date('Y'); // Si no se proporciona el año, usar el año actual

        $transacciones = Transaction::table('transacciones')
