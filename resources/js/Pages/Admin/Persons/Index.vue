...
const { hasRole } = usePermission();
    ...
    const showModalAlert     = ref(false);       // Variable para controlar la visibilidad del modal
    ...
    const selectedDate = ref(new Date());
     ...
     const modalTitleAlert = ref(''); 
    const showModalUpload = ref(false);

    const openModalAlert = (title) => {
  ...
  // Asegúrate de que 'formAlert' es un ref y usa .value
  formAlert.value.codigoSede = selectedRowsData.codigo_sede;
  formAlert.value.codigoCliente = selectedRowsData.codigo_cliente;
  formAlert.value.role = 'soporte'; // Asignar valor a role

  showModalAlert.value = true;  

  // Imprimir en la consola para depuración
  console.log('Código Sede:', formAlert.value.codigoSede);
  console.log('Código Cliente:', formAlert.value.codigoCliente);
  console.log('Rol:', formAlert.value.role);
};
...
/**************************************************************************************************************** */
    ...
    onBeforeMount(() => {
        ...
        userRole= hasRole;
        console.log(personas.value, userRole.value);
...
const deleteCliente = () => {
   ...
   // confirmation(contacto,  BASE_URL + '/persons' + id, '');
...
const openModalEdit = (title) => {
    ...
    formEdit.fecha=selectedRowsData.fecha,
...
const update = () => {
    ...
    if (selectedRows.length > 0) {
        ...
        console.log(formEdit);
    //  sendRequest('PUT',formEdit.value,( BASE_URL +  '/persons'+id),'');
...
const save = () => {
    ...
    //  sendRequest('PUT',form.value,(  BASE_URL + '/persons'+id),'');
...
/**************************************************************************************************************** */
    ...
    // Inicializar Pusher
        const pusher = new Pusher('2f8c4154a7ade73b67e2', {
        cluster: 'sa1'
        });
        ...
        channel.value = pusher.subscribe('alertas-channel.' + userRole.value);
        channel.value.bind('App\\Events\\AlertaCreada', (data) => {
            alert(`New alert: ${data.message} | Type: ${data.type} | Data: ${JSON.stringify(data.data)}`);
        });
        };

        const sendAlert = () => {
            // Imprimir cada variable del formulario en la consola
            console.log('Razón Social:', formAlert.value.razon_social);
            console.log('Mensaje de Alerta:', formAlert.value.alertMessage);
            console.log('Código Cliente:', formAlert.value.codigoCliente);
            console.log('Código Sede:', formAlert.value.codigoSede);
            console.log('Rol:', formAlert.value.role);
        ...
        //     clientCode: alertClientCode.value,
        //     role: roleSend.value, // Enviar alerta al rol 'soporte'
        // })
        // .then(response => {
...
<template>
      ...
      <div class="flex justify-center">
            <div class="w-full">
                <div class="bg-blue-400 border border-blue-400 text-blue-700 px-4 py-3 rounded relative" role="alert">
                    <div class="flex justify-around items-center"> <!-- Contenedor para los botones -->
                        ...
                        <button  @click="openModalAlert()" class="btn btn-lg block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                            <i class="fa-solid fa-circle-plus mx-1"></i> Enviar Alerta
                        </button>
                    </div>
                </div>
            </div>
        ...
        <!-- ************************************************************************************************************************************** -->         
        <div class="border px-5 py-5  border-primary-400 rounded-md  mt-3">    
  ...
  <!-- ************************************************************************************************************************************** -->
      ...
      <!-- ************************************************************************************************************************************** -->
        <!-- MODAL ALERT  -->
        <div v-show="showModalAlert" id="default-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto">
             <div class="bg-white rounded-lg shadow-lg max-w-md w-full h-screen-70 overflow-y-auto">
                <!-- Modal header -->
                <div class="flex items-center justify-between p-4 border-b">
                    <h3 class="text-xl font-semibold text-gray-900">
                    ...
                    <button @click="showModalAlert = false" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 flex justify-center items-center">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                        </svg>
                        <span class="sr-only">Cerrar Modal</span>
                ...
                <!-- Modal body -->
                <div class="p-4 space-y-4 overflow-y-auto">
                    <form @submit.prevent="sendAlert"  class="max-w-lg mx-auto border border-gray-300 p-6 my-5 rounded-lg overflow-y-auto">
                        <div class="my-2">
                            <h2 class="text-xl font-thin mb-4">{{ modalTitleAlert }}</h2>
                        </div>
                        <!-- Input para subir documentos -->
                       
                        <div class="mb-4">
                            <input hidden type="text" id="codigoCliente" class="form-input w-full" v-model="formAlert.razon_social">
                        </div>

                        <div class="mb-4">
                            <label for="alertMessage" class="block mb-1">MENSAJE</label>
                            <input type="text" name="alertMessage "id="alertMessage" class="form-input w-full" v-model="formAlert.alertMessage">
                            <p class="text-sm text-gray-500 mt-1">Ingrese razon social.</p>
                        </div>

                        
                        <!-- aqui pintar el rol del que envia la alerta -->
                        <div class="mb-4">
                            <input hidden type="text" id="codigoCliente" name="codigoCliente" class="form-input w-full" v-model="formAlert.codigoCliente">
                        </div>

                         <!-- aqui pintar el rol del que envia la alerta -->
                         <div class="mb-4">
                            <input hidden type="text" id="codigoSede" name="codigoSede" class="form-input w-full" v-model="formAlert.codigoSede">
                        </div>  

                        <!-- aqui pintar el rol del que envia la alerta -->
                        <div class="mb-4">
                            <input hidden type="text" id="role" name="role" class="form-input w-full" v-model="formAlert.role">
                        </div>

                        <!-- Agrega los demás campos del formulario con la misma estructura -->
                        <div class="flex items-center justify-end p-4 border-t">
                            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5">Subir Documento</button>
                            <button @click="showModalAlert = false" type="button" class="ml-3 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg hover:bg-white px-5 py-2.5">Rechazar</button>
                        </div>
                    </form>
                </div>
            </div>
